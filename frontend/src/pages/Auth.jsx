    ...
    try {
      if (isLogin) {
        ...
        if (error) {
          ...
          setIsLogin(true); // Switch to login view for them after registering
        }
      }
    } finally {
      setIsLoading(false);
      ...
      <div className="w-full max-w-md space-y-8 animate-fade-in">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          {!isLogin && (<div className="space-y-2">
            ...
            <Input id="name" type="text" placeholder="Your Name" value={name} onChange={(e) => setName(e.target.value)} required={!isLogin} className="h-12" />
          </div>)}
          ...
          <div className="space-y-2">
            ...
            <Input id="email" type="email" placeholder="abc@gmail.com" value={email} onChange={(e) => setEmail(e.target.value)} required className="h-12" />
          </div>
          ...
          <Button type="submit" size="lg" className="w-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white font-semibold py-3 rounded-lg transition-all duration-300" disabled={isLoading}>
        ...
        {isLogin && (<div className="mt-4">
          <div className="flex items-center justify-center w-full">
            <button onClick={handleSignIn} className="w-full px-6 py-2.5 bg-transparent border-2 border-teal-400 text-teal-400 font-semibold rounded-lg hover:bg-teal-400 hover:text-white transition-all duration-200 ease-in-out">
              Sign In
            </button>
          </div>
        </div>)}
